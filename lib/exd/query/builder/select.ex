defmodule Exd.Query.Builder.Select do
  ...
  def escape({binding, _, nil}), do: {:binding, Atom.to_string(binding), []}
  ...
  defp escape_arg({binding, _, nil} = expr) when is_atom(binding), do: {:binding, Atom.to_string(binding), nil}
  defp escape_arg({binding, _, nil}), do: {:binding, Atom.to_string(binding), []}
  defp escape_arg({{:., _, [{binding, _, nil} | [path]]}, _, []}) do
    {:binding, Atom.to_string(binding), Atom.to_string(path)}
  end
