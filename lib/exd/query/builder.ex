defmodule Exd.Query.Builder do
  ...
  """
  alias Exd.Query
  alias Exd.Query.Builder.{From, Where, Select, Join, Into}
  ...
  defp from([{type, expr} | rest], quoted, binds, env) when type in @joins do
    quoted =
      quote do
        Exd.Query.Builder.join(unquote(quoted), unquote(type), unquote(binds), unquote(expr))
      end
  ...
  """
  defmacro join(query, type, binds \\ [], expr) do
    Join.build(query, type, binds, expr, __CALLER__)
  end
